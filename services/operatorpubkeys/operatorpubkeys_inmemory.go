...
func NewOperatorPubkeysServiceInMemoryWithStart(
	ctx context.Context,
	avsRegistrySubscriber avsregistry.AvsRegistrySubscriber,
	avsRegistryReader avsregistry.AvsRegistryReader,
	logger logging.Logger,
	...
	wg := sync.WaitGroup{}
	wg.Add(1)
	pkcs.startServiceInGoroutineWithStart(ctx, queryC, &wg, start)
	wg.Wait()
	return pkcs
}

func (ops *OperatorPubkeysServiceInMemory) startServiceInGoroutineWithStart(ctx context.Context, queryC <-chan query, wg *sync.WaitGroup, start uint64) {
	go func() {
		...
		newPubkeyRegistrationC, newPubkeyRegistrationSub, err := ops.avsRegistrySubscriber.SubscribeToNewPubkeyRegistrationsWithStart(start)
		if err != nil {
		...
		bigInt := *new(big.Int).SetUint64(start)
		ops.queryPastRegisteredOperatorEventsAndFillDbWithStart(ctx, pubkeyDict, bigInt)
...
func (pkcs *OperatorPubkeysServiceInMemory) queryPastRegisteredOperatorEventsAndFillDbWithStart(ctx context.Context, pubkeydict map[common.Address]types.OperatorPubkeys, start big.Int) {
	// Querying with nil startBlock and stopBlock will return all events. It doesn't matter if we query some events that we will receive again in the websocket,
	// since we will just overwrite the pubkey dict with the same values.
	alreadyRegisteredOperatorAddrs, alreadyRegisteredOperatorPubkeys, err := pkcs.avsRegistryReader.QueryExistingRegisteredOperatorPubKeys(ctx, &start, nil)
	if err != nil {
		pkcs.logger.Error("Fatal error querying existing registered operators", "err", err, "service", "OperatorPubkeysServiceInMemory")
		panic(err)
	}
